...
def create_app(global_tree=None, test_config=None):
    ...
    def rt():
        ...
        if 'json' in request.args.get('render', []):
            return jsonify({"tag": global_tree_recv["metadata"]["count"]["tags"],
                            "label": global_tree_recv["metadata"]["count"]["labels"],
                            "test": global_tree_recv["metadata"]["count"]["tests"],
                            "files": global_tree_recv["metadata"]["count"]["files"]})
        return render_template('realtime.html',
                               rootdir=global_tree_recv['metadata']['rootdir'],
                               nb_tests=global_tree_recv['metadata']['count']['tests'],
                               nb_labels=global_tree_recv['metadata']['count']['labels'],
                               nb_tags=global_tree_recv['metadata']['count']['tags'],
                               nb_files=global_tree_recv['metadata']['count']['files'])
    ...
    @app.route("/realtime/<selection>/list")
    def get_list_realtime(selection):
        """Get a listing.
        ...
        """
        if 'json' not in request.args.get('render', []):
            return render_template('list_view_realtime.html', selection=selection)

        out = list()
        for name, value in global_tree_recv[selection].items():
            out.append({
                "name": name,
                "count": value['metadata']['count']
            })
    ...
    @app.route("/realtime/<selection>/detail")
    def get_details_realtime(selection):
        """Get a detailed view of a component.
        ...
        if 'json' not in request.args.get('render', []):
            return render_template("detailed_view_realtime.html",
                                   selection=selection,
                                   sel_item=request_item)

        if request_item in global_tree_recv[selection].keys():
            for test in global_tree_recv[selection][request_item]['tests']:
                out.append(test)
        return jsonify(out)
    ...
    def submit():
        if request.method == "GET":
            ...
            global_tree_recv["metadata"] = to_add["metadata"]
            return "This page is not meant to be visited"

    def addToGlobalTree(to_add, sorting_type, names):
        if sorting_type not in global_tree_recv:
            global_tree_recv["sorting_type"] = {}
        for name in names:
            if name not in global_tree_recv[sorting_type]:
                global_tree_recv[sorting_type][name] = {"tests": []}
                global_tree_recv[sorting_type][name]["metadata"] = {
                    "count": {
                        "WAITING": 0,
                        "IN_PROGRESS": 0,
                        "SUCCEED": 0,
            ...
            global_tree_recv[sorting_type][name]["tests"].append(to_add["test_data"])
            global_tree_recv[sorting_type][name]["metadata"]["count"][to_add["state"]] += 1
            global_tree_recv[sorting_type][name]["metadata"]["count"]["total"] += 1

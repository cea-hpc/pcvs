...
def load_session_file():
    global _defined_sessions
    sessions_file = PATH_SESSION
    with open(sessions_file, 'r') as fh:
        _defined_sessions = yaml.safe_load(fh)
...
class Session:
    ...
    def __check_defined_program_validity(self):
    ...
    def prepare(self):
        ...
        self.__check_defined_program_validity()
    ...
    def find_files_to_process(self, path_dict):
        setup_files = list()
        yaml_files = list()
    ...
    def process(self):
        ...
        setup_files, yaml_files = self.find_files_to_process(
        ...
        errors = []
        errors += self.process_dyn_setup_scripts(setup_files)
        errors += self.process_static_yaml_files(yaml_files)
    ...
    def build_env_from_configuration(self, current_node, parent_prefix="pcvs"):
        env_dict = dict()
        for k, v in current_node.items():
            if v is None:
                v = ''
            if isinstance(v, dict):
                env_dict.update(self.build_env_from_configuration(
                    v, parent_prefix+"_"+k))
                continue
            elif v is None:
                v = ''
    ...
    def process_dyn_setup_scripts(self, setup_files):
        err = []
        log.print_item("Convert configuation to Shell variables")
        env = os.environ.copy()
        env.update(self.build_env_from_configuration(MetaConfig.root))
    ...
    def process_static_yaml_files(self, yaml_files):
        err = []
    ...
    def save_for_export(self, f, dest=None):
        config = MetaConfig.root.validation
        ...
        for root, _, files in os.walk(os.path.join(outdir, "test_suite")):
            for file in files:
                ...
                if file.endswith(('.json', '.xml', '.yml')):
                    self.save_for_export(os.path.join(root, file))

        self.save_for_export(os.path.join(outdir, 'conf.yml'))
    ...
    def dup_another_build(self, build_dir, outdir):
        global_config = None

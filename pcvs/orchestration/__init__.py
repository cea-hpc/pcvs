...
class Orchestrator:
    ...
    def start_run(
        ...
        self._publisher.flush()
        assert (
            self._manager.get_count("executed") == self._manager.get_count("total") or interrupted
        )
        ...
        failed: int = self._manager.get_count("total") - self._manager.get_count(
            str(TestState.SUCCESS)
        )
